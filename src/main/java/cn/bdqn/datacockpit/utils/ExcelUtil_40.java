...
public class ExcelUtil_40 {  
    ...
    public ExcelUtil_40(String excelPath){  
     ...
     * @return 
     */  
    public ExcelUtil_40 RestoreSettings(){  
        ExcelUtil_40 instance = new  ExcelUtil_40(this.excelPath);  
        return instance;  
    }  
    ...
    public   List<Row> addExecl(String excelPath){
    	ExcelUtil_40 eu = new ExcelUtil_40();  
    	
        //从第一行开始读取  
        eu.setStartReadPos(0);  
        eu.setExcelPath(excelPath);
        //String src_xlspath = "C:\\Users\\admin\\Desktop\\123.xls";  
        List<Row> row = null;  
        try {  
            row = eu.readExcel(excelPath);  
         
      //读取对象
       for (Row row2 : row) {
    	   //取出每行数据
    	   Iterator<Cell> iterator = row2.iterator();
    	   //遍历行内每个元素数据
