...
public class BaseDao {
	 ...
	 * @throws Exception
	 */
	public <T> List<T> executeQueryObject(Class<T> clazz, String sql,
			...
			for (Field field : fields) {
				...
				for (int i = 0; i < metaData.getColumnCount(); i++) {
					...
					String fieldName = metaData.getColumnName(i + 1)
							.replaceAll("[-_]", "");
					// 匹配字段名和属性名
					if (fieldName.equalsIgnoreCase(field.getName().replaceAll(
							"[-_]", ""))) {
						// 获取字段的值
						Object object = rs.getObject(metaData
								.getColumnName(i + 1));
	 ...
	 * @throws Exception
	 */
	public <T> List<Map<String, Object>> executeQueryMap(String sql,
			Object... params) throws Exception {
		// 调用查询的方法得到rs
		this.executeQuery(sql, params);
		// 创建list用于返回
