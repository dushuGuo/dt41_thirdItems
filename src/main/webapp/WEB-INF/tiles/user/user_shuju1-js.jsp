	  ...
	  $.post("shuju_1.shtml",{"aid":id},function(data){
		  ...
		  $.each(data,function(eve){
			 var str = '<tr><th style="text-align: center;">'+(eve+1)+'</th>'
                 +'<th style="text-align: center;">'+(this.starttime==null?"":new Date(this.starttime).getFullYear()+"-"+(new Date(this.starttime).getMonth()+1)+"-"+new Date(this.starttime).getDate()+" "+new Date(this.starttime).getHours()+":"+new Date(this.starttime).getMinutes()+":"+new Date(this.starttime).getSeconds())+'</th>'
                 +'<th style="text-align: center;">'+(this.endtime==null?"":new Date(this.endtime).getFullYear()+"-"+(new Date(this.endtime).getMonth()+1)+"-"+new Date(this.endtime).getDate()+" "+new Date(this.endtime).getHours()+":"+new Date(this.endtime).getMinutes()+":"+new Date(this.endtime).getSeconds())+'</th>'
