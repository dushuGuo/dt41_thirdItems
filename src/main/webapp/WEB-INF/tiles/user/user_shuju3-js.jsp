...
$("#write").click(function(){
     ...
     var urlinfo=window.location.href//获取当前地址
  	 var offset=urlinfo.indexOf("?id=");//设置参数字符串开始的位置
  
  	 var tableName=urlinfo.substr(offset+4)//取出参数字符串 这里会获得类似“id=1”这样的字符串
  	 var destr = UrlDecode(tableName);
     window.location.href="excle.shtml?tableName="+destr; 
});

  $(function () {
	  
	     var urlinfo=window.location.href//获取当前地址
	  	 var offset1=urlinfo.indexOf("?id=");//设置参数字符串开始的位置
	  	 var offset2=urlinfo.indexOf("&infoId=");
	  	 var tableName=urlinfo.substring(offset1+4,offset2);
	  	 var destr = UrlDecode(tableName);
	     $("input[type=hidden]").attr("value",destr);
